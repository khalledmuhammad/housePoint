...
const ListingDynamicDetailsV1 = () => {
  ...
  useEffect(() => {
    async function getPageData() {
      const apiUrlEndpoint = `http://localhost:5000/api/prop/${id}`;
    ...
    async function getImageData() {
      const apiUrlEndpoint = `http://localhost:5000/api/image/${id}`;
      const { data } = await axios.get(apiUrlEndpoint);
     console.log(data)
     setImage(data)
    }
                        ...
                        {({ ref, open }) => (
                          <div role="button" ref={ref} onClick={open}>
                            <img
                              ...
                              src={`https://housepointegypt.com/photos/${ image && image[0].file_image}`}
                              alt="1.jpg"
                            />
                          </div>
                        )}
